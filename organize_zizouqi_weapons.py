...
OUT_DIR = Path("autochess_dump/organized_weapons")
...
def make_contact_sheet(manifest: list[dict], out_path: Path) -> None:
    ...
    for i, row in enumerate(manifest[:count]):
        ...
        card = Image.open(row["weapon_png"]).convert("RGBA")
        paste_center(sheet, fit_image(card, (thumb_w - 10, thumb_h - 10)), (x, y, x + thumb_w, y + thumb_h))
        label = row["base_resource"].replace("Icon_ZiZouQi_WuQi_", "").replace("Icon_ZiZouQi_Weapon_", "")
...
def main() -> None:
    ...
    weapon_cards = [
        row
        for row in rows
        if row.get("crop_png") and row.get("card_image_type") in {"weapon_card", "weapon"}
    ]

    if OUT_DIR.exists():
        shutil.rmtree(OUT_DIR)

    cards_dir = OUT_DIR / "weapon_cards"
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    manifest: list[dict] = []
    for index, row in enumerate(sorted(weapon_cards, key=lambda item: item["resource"]), 1):
        base = base_resource(row["resource"])
        out_path = copy_png(row.get("crop_png"), cards_dir / f"{index:03d}_{safe_name(base)}.png")
        manifest.append(
            {
                "index": index,
                "base_resource": base,
                "weapon_resource": row["resource"],
    ...
    (OUT_DIR / "weapons_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    with (OUT_DIR / "weapons_manifest.csv").open("w", encoding="utf-8-sig", newline="") as f:
        fieldnames = ["index", "base_resource", "weapon_resource", "weapon_png", "source_png", "atlasAssetName"]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for row in manifest:
            writer.writerow({key: row.get(key) for key in fieldnames})

    make_contact_sheet(manifest, OUT_DIR / "weapon_cards_check_contact.png")

    print(f"weapon_cards={len(manifest)}")
