...
OUT_DIR = Path("autochess_dump/organized_items")
...
def make_contact_sheet(manifest: list[dict], out_path: Path) -> None:
    ...
    for i, row in enumerate(manifest[:count]):
        ...
        image = Image.open(row["item_png"]).convert("RGBA")
        paste_center(sheet, fit_image(image, (thumb_w - 10, thumb_h - 10)), (x, y, x + thumb_w, y + thumb_h))
        label = row["item_resource"].replace("Icon_ZiZouQi_DaoJu_", "")
...
def main() -> None:
    ...
    items = [
        row
        for row in rows
        if row.get("crop_png") and row.get("card_image_type") == "item_card"
    ]

    if OUT_DIR.exists():
        shutil.rmtree(OUT_DIR)

    cards_dir = OUT_DIR / "item_cards"
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    manifest: list[dict] = []
    for index, row in enumerate(sorted(items, key=lambda item: item["resource"]), 1):
        out_path = copy_png(row.get("crop_png"), cards_dir / f"{index:03d}_{safe_name(row['resource'])}.png")
        manifest.append(
            {
                "index": index,
                "item_resource": row["resource"],
    ...
    (OUT_DIR / "items_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    with (OUT_DIR / "items_manifest.csv").open("w", encoding="utf-8-sig", newline="") as f:
        fieldnames = ["index", "item_resource", "item_png", "source_png", "atlasAssetName"]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for row in manifest:
            writer.writerow({key: row.get(key) for key in fieldnames})

    make_contact_sheet(manifest, OUT_DIR / "item_cards_check_contact.png")

    print(f"item_cards={len(manifest)}")
